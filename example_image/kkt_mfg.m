function [res_stat, res_feas] = kkt_mfg(rho,mx,my,opts)
res_feas = zeros(2,1);
...
phi = trans_dct( Dtadj(grad_rho)+Dxadj(grad_mx)+Dyadj(grad_my) )./lap;
phi(1,1,1) = 0;
phi = trans_idct(phi);

err_stat = {grad_rho - cat(1,Dt(phi),-phi(end,:,:)/dt),...
            grad_mx - Dx(phi), grad_my - Dy(phi)};
