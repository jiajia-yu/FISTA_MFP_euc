...
if isfield(opts,'L0')    L0    = opts.L0; else L0  = 8; end
if isfield(opts,'eta')   eta = opts.eta;  else eta = 1.5; end 
...
%                 + L/2*sum(diff_my .*diff_my, 'all');
    ...
    diff_rho = xrho_k - xrho_km1;
    diff_mx = xmx_k - xmx_km1;
    diff_my = xmy_k - xmy_km1;
    yrho = max( xrho_k + w_k*diff_rho,0.1);
%     yrho = xrho_k + w_k*(xrho_k-xrho_km1);
    ymx  = xmx_k  + w_k*diff_mx;
    ymy  = xmy_k  + w_k*diff_my;
    ...
    % stop criteria
    res = sqrt( dt*dx*dy*( norm(diff_rho(:))^2+norm(diff_mx(:))^2+norm(diff_my(:))^2 ) );
    if res < tol
        break
    end
