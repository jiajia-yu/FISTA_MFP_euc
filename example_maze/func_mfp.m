function [rho,mx,my,outs] = func_mfp(rho0,rho1,opts)
...
%         ind = rho ~=0;
        ...
        obj = lambda_L*sum( (mx(ind).^2+my(ind).^2)./rho(ind) ) ...
             +lambda_E*sum( rho(ind).*log(rho(ind)) ) ...
        	 +lambda_P*sum( rho(ind).*Qx(ind) );
    end
...
%         ind = rho ~=0;
        ...
        grad_rho(ind) = - lambda_L*(mx(ind).^2+my(ind).^2)./rho(ind).^2/2 ...
            + lambda_E*(log(rho(ind))+1) + lambda_P*Qx(ind);
