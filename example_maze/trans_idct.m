...
if precompute,
    ...
    ww{1} = cos( pi/(4*siz(1)+2) * (1:2:2*siz(1)-1)' * (2*siz(1)-1:-2:1) );
    ww{1} = ww{1} * (2/sqrt(2*siz(1)+1));
...
if ndim==2
    ...
    else a = idct(itrans(a,ww{1}).',ww{2},ind{2}).'; % 2D case
    end       
else
    % ND case (3D and higher)
    a = reshape(itrans(reshape(a,siz(1),[]),ww{1}),siz);
    siz = [siz(2:end) siz(1)];
    a = shiftdim(a,1);
    for i=2:ndim
